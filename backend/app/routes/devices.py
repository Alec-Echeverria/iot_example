...
async def crear_dispositivo(
    ...
    # ✅ 5. Registrar el dispositivo con estado "offline"
    nuevo_dispositivo = {
        "name": dispositivo.name,
        "device_id": dispositivo.device_id,
        "username": user["username"],
        "mqtt_username": mqtt_username,
        "status": "offline"  # <- Aquí lo estableces por defecto
    }
    res = await db["dispositivos"].insert_one(nuevo_dispositivo)

    # 6. Crear regla en EMQX
    ...
    try:
        ...
        await db["emqx_save_rules"].insert_one({
            ...
            "payload_tmpl": new_rule["actions"][0]["params"]["payload_tmpl"],
            "enabled": True,
        })
        ...
        raise HTTPException(
            status_code=502,
            detail="Dispositivo creado, pero falló la creación de la regla en EMQX"
        )
...
async def obtener_dispositivos(user: dict = Depends(get_current_user)):
    ...
    async for device in devices_cursor:
        devices.append({
            ...
            "mqtt_username": device.get("mqtt_username", "-"),     # <-- agregado
            "status": device.get("status", "offline").lower()
        })
    return devices
